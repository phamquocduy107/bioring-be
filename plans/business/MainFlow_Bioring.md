...
5. **Màn hình 3 - Advanced Design (SW only)** (incremental save từng bước nhỏ):
   ...
   - **Bước 5a - Ghi âm:** User thu âm 5-15 giây. Mobile upload MP3 lên Cloudinary → nhận `audioUrl`. Gọi `PATCH /api/v1/engravings/versions/:versionId/config { customizationConfig: { ... }, audioUrl }`. (`audioUrl` là field **riêng**, không nằm trong `customizationConfig`.)
...
7. Store Staff hỗ trợ lấy dữ liệu (vân tay, giọng nói, nhịp tim) thông qua IoT Device và upload lên System. Server tự động xử lý SW → waveform SVG, FP → fingerprint SVG.
