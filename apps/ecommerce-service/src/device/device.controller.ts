...
export class DeviceController {
  ...
  async listDevices(data: { page?: number; limit?: number; status?: string; search?: string }) {
    try {
  ...
  async updateDevice(data: { id: string; device_name?: string; device_type?: string; status?: string; firmware_version?: string }) {
    try { return await this.service.updateDevice(data); }
    catch (e: any) { throw new RpcException(e); }
  }

  @GrpcMethod('EcommerceService', 'DeleteDevice')
  async deleteDevice(data: { id: string }) {
    try { return await this.service.deleteDevice(data.id); }
    catch (e: any) { throw new RpcException(e); }
  }
}
