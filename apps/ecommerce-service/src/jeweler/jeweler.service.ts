...
export class JewelerService {
  ...
  async getMyPerformance(jewelerId: string, fromDate?: string) {
    ...
    const qaPassed = qaChecks.filter(t => t.qa_checks.some(q => q.result === 'PASSED')).length;
    ...
    // recent tasks
    const recentWhere: any = { assigned_jeweler_id: jewelerId, status: 'COMPLETED' };
    ...
    return {
      ...
      qa_pass_rate: qaChecks.length > 0 ? Number(((qaPassed / qaChecks.length) * 100).toFixed(1)) : 0,
